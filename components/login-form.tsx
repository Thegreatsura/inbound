...
export function LoginForm({
	...
	const handlePasskeySignIn = async () => {
		...
		try {
			...
			// Show dedicated error state for passkey not found
			if (message.includes("not found")) {
				onPasskeyError?.(
					"We couldn't find a passkey for your account. You may need to sign in with another method first, then add a passkey in your settings.",
				);
				return;
			}
	...
	return (
		...
		>
			...
			<div className="grid gap-3">
				...
				<Button
					type="button"
					variant={lastUsedMethod === "google" ? "primary" : "secondary"}
					className="w-full"
				...
				<Button
					type="button"
					variant={lastUsedMethod === "github" ? "primary" : "secondary"}
					className="w-full"
