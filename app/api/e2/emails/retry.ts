...
export const retryEmail = new Elysia().post(
  ...
  async ({ request, params, body, set }) => {
    ...
    if (receivedEmail.length > 0) {
      ...
      if (body.delivery_id) {
        ...
        if (deliveryRecord.status === "success") {
          ...
          set.status = 400;
          return {
            error: "Delivery already succeeded. Use resend to deliver again.",
          };
        }
        ...
        try {
          ...
          console.log(
            `✅ Retry delivery successful for delivery ${body.delivery_id}`
          );
      ...
      console.log(
        `🔄 Retrying to all configured endpoints for email ${email.emailId}`
      );
    ...
    if (sentEmail.length > 0) {
      ...
      console.log(
        "⚠️ Cannot retry sent emails - use send endpoint to send a new email"
      );
      set.status = 400;
      return {
        error:
