...
export const createEmailAddress = new Elysia().post(
  ...
  async ({ request, body, set }) => {
    ...
    const domainResult = await db
      .select()
      .from(emailDomains)
      .where(and(eq(emailDomains.id, body.domainId), eq(emailDomains.userId, userId)))
      .limit(1)
    ...
    if (body.endpointId) {
      ...
      const endpointResult = await db
        .select()
        .from(endpoints)
        .where(and(eq(endpoints.id, body.endpointId), eq(endpoints.userId, userId)))
        .limit(1)
      ...
      const webhookResult = await db
        .select()
        .from(webhooks)
        .where(and(eq(webhooks.id, body.webhookId), eq(webhooks.userId, userId)))
        .limit(1)
    ...
    console.log("✅ POST /api/e2/email-addresses - Successfully created email address")
    set.status = 201
    return response
  },
  {
