...
export async function POST(
  ...
  console.log(
    "🧪 POST /api/v2/endpoints/{id}/test - Starting test for endpoint:",
    id
  );
  ...
  try {
      ...
      case "webhook":
        try {
          const effectiveUrl = resolveEffectiveUrl(
            requestData.overrideUrl,
            config.url
          );
          console.log(
            "🔗 Testing webhook endpoint:",
            maskUrl(effectiveUrl),
            requestData.overrideUrl ? "(override)" : ""
          );
    ...
    console.error(
      "💥 Unexpected error in POST /api/v2/endpoints/{id}/test:",
      error
    );
    return NextResponse.json(
      {
