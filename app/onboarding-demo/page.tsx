        ...
        <div className="flex-1">
          ...
          >
            <div className="flex items-center gap-3 mb-4">
              <h2 className="text-xl font-semibold text-foreground tracking-[-0.02em]">Create an inbound API key</h2>
              {apiKeyCreated ? (
            ...
            <p className="text-sm text-muted-foreground font-medium tracking-[-0.02em] mb-6">
              This will enable you to send and receive emails through inbound's API with full webhook support.
            </p>
              ...
              <div className="animate-in slide-in-from-top-2 duration-500">
                <div className="rounded-lg p-4 px-[0] py-[0] border-0">
                  <div className="flex items-center justify-between mb-2">
                    <span className="text-xs text-muted-foreground font-medium tracking-[-0.02em]">API Key</span>
                  </div>
          ...
          >
            <div className="flex items-center gap-3 mb-4">
              <h2 className="text-xl font-semibold text-foreground tracking-[-0.02em]">Send an email</h2>
              {emailSent ? (
                <div className="flex items-center gap-2 bg-emerald-50 dark:bg-emerald-950/50 text-emerald-700 dark:text-emerald-400 px-3 py-1 rounded-full text-xs font-medium border border-emerald-200 dark:border-emerald-800">
                  <span>Sent</span>
                  <Check2 className="w-3 h-3" />
            ...
            <p className="text-sm text-muted-foreground font-medium tracking-[-0.02em] mb-6">
              Now you can send your first email! Just click the run button below.
            </p>
            ...
            <div className="bg-gray-900 dark:bg-black rounded-lg p-6 font-mono text-xs text-white relative">
              <div className="absolute top-4 right-4 flex gap-2">
                <Button
                  ...
                  disabled={isRunning || !createdApiKey || !demoEmail || currentStep < 2}
                >
              ...
              <div className="space-y-2 pr-20">
                <div className="text-gray-400">// Now you can send your first email!</div>
                <div className="text-gray-400">// Just click the run button below!</div>
                ...
                <div className="ml-4">
                  apiKey: <span className="text-green-400">'{createdApiKey ? createdApiKey.slice(0, 8) + '...' : 'your-api-key'}'</span>
                </div>
                <div>{"}"}</div>
                <div className="mt-2"></div>
                <div className="text-gray-400">// Simple email</div>
                <div>
                  <span className="text-blue-400">const</span> {"{ id }"} = <span className="text-blue-400">await</span>{" "}
                  inbound.emails.<span className="text-yellow-400">send</span>({"{"})
                </div>
                <div className="ml-4">
                  from: <span className="text-green-400">'agent@inbnd.dev'</span>,
                </div>
                ...
                <div className="ml-4">
                  text:{" "}
                  <span className="text-green-400">'Thanks for signing up! What is your favorite email client?'</span>,
                </div>
                <div className="ml-4">
                  html:{" "}
                  <span className="text-green-400">
                    '&lt;p&gt;Thanks for signing up! What is your favorite email client?&lt;/p&gt;'
                  </span>
                </div>
                <div>{"}"}</div>
                <div className="mt-2"></div>
          ...
          >
            <div className="flex items-center gap-3 mb-4">
              <h2 className="text-xl font-semibold text-foreground tracking-[-0.02em]">Running...</h2>
            ...
            <div className="bg-black rounded-lg p-6 font-mono text-xs text-green-400">
              <div className="space-y-1">
                {demoOutput ? (
                  <>
                    <div className="text-muted-foreground mb-1">$ inbound.emails.send( {demoEmail} )</div>
                    <div className="whitespace-pre-wrap text-green-400">{demoOutput}</div>
                    {isListeningForReply && (
                      ...
                      <div className="mt-3 bg-gray-900 border border-gray-700 rounded p-3">
                        ...
                        <div className="text-white text-xs space-y-1">
                          <div className="flex gap-2">
                            <span className="text-blue-400 font-medium min-w-[50px]">From:</span>
                            <span className="text-yellow-400">{replyReceived.from}</span>
                          </div>
                          <div className="flex gap-2">
                            <span className="text-blue-400 font-medium min-w-[50px]">Subject:</span>
                            <span className="text-yellow-400">{replyReceived.subject}</span>
                          </div>
                          <div className="flex gap-2">
                            <span className="text-blue-400 font-medium min-w-[50px]">Body:</span>
                  ...
                  <>
                    ...
                    {!emailReceived && <div className="animate-pulse">Awaiting reply...</div>}
                    {emailReceived && (
                      <>
                        <div>✓ Email received!</div>
                        <div className="text-yellow-400">Subject: Re: Welcome to Inbound</div>
                        <div className="text-yellow-400">From: user@example.com</div>
                      </>
                    )}
                  </>
                ) : (
                  <div className="text-muted-foreground">Terminal output will appear here...</div>
                )}
              </div>
            </div>
          </div>
          ...
          >
            <div className="flex items-center gap-3 mb-4">
              <h2 className="text-xl font-semibold text-foreground tracking-[-0.02em]">Email received!</h2>
              {emailReceived ? (
                <div className="flex items-center gap-2 bg-emerald-50 dark:bg-emerald-950/50 text-emerald-700 dark:text-emerald-400 px-3 py-1 rounded-full text-xs font-medium border border-emerald-200 dark:border-emerald-800">
                  <span>Success</span>
                  <Check2 className="w-3 h-3" />
            ...
            <p className="text-sm text-muted-foreground font-medium tracking-[-0.02em] mb-6">
              Great! You've received a typesafe webhook with full email data - HTML, plain text, headers, and more.
            </p>
                ...
                <div className="bg-muted/50 rounded-lg p-6 border border-border">
                  <div className="space-y-3">
                    <div className="flex justify-between items-start">
                      <div>
                        <div className="font-semibold text-sm text-foreground">{replyReceived.subject}</div>
                        <div className="text-xs text-muted-foreground">From: {replyReceived.from}</div>
                      </div>
          ...
          <div className="space-y-8">
            <div className="bg-card rounded-lg p-4 border border-border shadow-sm">
              ...
              <p className="text-xs text-muted-foreground font-medium tracking-[-0.02em]">
                Send and receive emails via SDK/API with typesafe webhooks. Get full email data including HTML, plain
                text, raw content, and headers.
              </p>
            </div>

            <div className="bg-card rounded-lg p-4 border border-border shadow-sm">
              <div className="font-semibold text-sm text-foreground mb-2">Typesafe Webhooks</div>
              <p className="text-xs text-muted-foreground font-medium tracking-[-0.02em]">
                Receive fully typed webhook payloads with complete email information. No guessing - you get every detail
                for maximum observability.
              </p>
            </div>

            <div className="bg-card rounded-lg p-4 border border-border shadow-sm">
              <div className="font-semibold text-sm text-foreground mb-2">Maximum Observability</div>
              <p className="text-xs text-muted-foreground font-medium tracking-[-0.02em]">
                Access HTML content, plain text, raw email data, and full headers. Perfect for debugging, analytics, and
                building intelligent email workflows.
              </p>
            </div>

            <div className="bg-card rounded-lg p-4 border border-border shadow-sm">
              <div className="font-semibold text-sm text-foreground mb-2">Quick Integration</div>
              <p className="text-xs text-muted-foreground font-medium tracking-[-0.02em]">
                Get started in minutes with inbound's simple SDK. Send your first email and start receiving webhooks
                immediately.
              </p>
            </div>
          </div>
