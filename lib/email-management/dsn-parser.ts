...
import { emailDomains, sentEmails, sesTenants, user } from "@/lib/db/schema";
...
export function isDsn(
	...
	if (headers?.["content-type"]) {
		...
		if (typeof contentType === "object" && contentType !== null) {
			const typedContentType = contentType as {
				value?: string;
				params?: Record<string, string>;
			};
			if (
				typedContentType.value === "multipart/report" &&
				typedContentType.params?.["report-type"] === "delivery-status"
			) {
				return true;
			}
		}
...
function extractMultipartPartBody(part: string): string | null {
	const separator = "\r\n\r\n";
	const separatorAlt = "\n\n";

	const separatorIndex = part.indexOf(separator);
	if (separatorIndex !== -1) {
		return part.substring(separatorIndex + separator.length).trim();
	}

	const separatorAltIndex = part.indexOf(separatorAlt);
	if (separatorAltIndex !== -1) {
		return part.substring(separatorAltIndex + separatorAlt.length).trim();
	}
...
function extractDeliveryStatusPart(rawContent: string): string | null {
	...
	for (const part of parts) {
		if (
			...
			const partBody = extractMultipartPartBody(part);
			if (partBody) {
				return partBody;
			}
		}
	...
	for (const section of sections) {
		...
		for (const line of lines) {
			...
			switch (normalizedField) {
				...
				case "action":
					result.action =
						fieldValue.toLowerCase() as DSNDeliveryStatus["action"];
					break;
				...
				case "original-message-id":
					result.originalMessageId = fieldValue.trim().replace(/^<|>$/g, "");
					break;
				case "status":
 ...
 */
function extractDsnReplyHeaders(rawContent: string): {
	inReplyTo?: string;
	references?: string[];
} {
	...
	const unfoldedHeaders = headersSection.replace(/\r?\n[ \t]+/g, " ");

	// Extract In-Reply-To header
	// Format: In-Reply-To: <message-id@domain.com>
	const inReplyToMatch = unfoldedHeaders.match(
		/^In-Reply-To:\s*<?([^>\r\n]+)>?\s*$/im,
	);
	if (inReplyToMatch) {
	...
	const referencesMatch = unfoldedHeaders.match(/^References:\s*(.+)$/im);
	if (referencesMatch) {
...
export async function parseDsn(rawContent: string): Promise<ParsedDSN> {
	...
	if (deliveryStatusPart) {
		result.deliveryStatus =
			parseDeliveryStatus(deliveryStatusPart) || undefined;
	}
	...
	if (originalMessagePart) {
		result.originalMessage =
			parseOriginalMessage(originalMessagePart) || undefined;
	}
	...
	if (result.deliveryStatus?.status) {
		...
		result.bounceReason =
			result.deliveryStatus.diagnosticCode || result.statusDescription;
	}
 ...
 */
export async function getDsnSourceInfo(
	dsn: ParsedDSN,
): Promise<DSNSourceInfo | null> {
	...
	for (const candidate of messageIdCandidates) {
		...
		dedupedCandidates.push(normalizedCandidate);
	}
	...
	try {
		...
		for (const candidate of dedupedCandidates) {
			const messageIdVariations = new Set<string>();
			messageIdVariations.add(candidate);
			messageIdVariations.add(`<${candidate}>`);

			const atIndex = candidate.indexOf("@");
			if (atIndex !== -1) {
				const localPart = candidate.substring(0, atIndex);
				if (localPart) {
					messageIdVariations.add(localPart);
					messageIdVariations.add(`<${localPart}>`);
				}
			} else {
				messageIdVariations.add(`${candidate}@us-east-2.amazonses.com`);
				messageIdVariations.add(`<${candidate}@us-east-2.amazonses.com>`);
			}

			const lookupValues = Array.from(messageIdVariations);

			const [matchedEmail] = await db
				.select({
					id: sentEmails.id,
				...
				.where(
					or(
						...lookupValues.map((mid) => eq(sentEmails.sesMessageId, mid)),
						...lookupValues.map((mid) => eq(sentEmails.messageId, mid)),
					),
				)
		...
		const result: DSNSourceInfo = {
			...
			triggeringEmailMessageId:
				email.sesMessageId || email.messageId || undefined,
		...
		if (email.fromDomain) {
			const domainInfo = await db
				...
				.where(
					and(
						eq(emailDomains.domain, email.fromDomain),
						eq(emailDomains.userId, email.userId),
					),
				)
				.limit(1);
		...
		if (!result.tenantId && email.userId) {
			const tenantInfo = await db
				.select({
					id: sesTenants.id,
					tenantName: sesTenants.tenantName,
				...
				.from(sesTenants)
				.where(eq(sesTenants.userId, email.userId))
				.limit(1);
