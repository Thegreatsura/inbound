...
export async function storeSESEvent(params: {
  ...
  try {
    ...
    console.log(`📊 storeSESEvent - Stored ${params.eventType} event: ${eventId}`)
...
export async function getTenantRates(configurationSetName: string): Promise<TenantRates | null> {
  try {
    ...
    if (!tenant) {
      console.log(`⚠️ getTenantRates - Tenant not found for config set: ${configurationSetName}`)
      return null
    }
...
export async function processSESEvent(params: {
  ...
  if (!storeResult.success) {
    console.error(`❌ processSESEvent - Failed to store event: ${storeResult.error}`)
  ...
  if (alerts.length > 0) {
    console.log(`🚨 processSESEvent - ${alerts.length} alert(s) triggered for ${params.configurationSetName}`)
