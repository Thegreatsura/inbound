...
export const guardKeys = {
	...
	list: (filters?: Record<string, string>) =>
		[...guardKeys.lists(), filters] as const,
	...
	>({
		...
		onSuccess: (_, variables) => {
			...
			queryClient.invalidateQueries({
				queryKey: guardKeys.detail(variables.ruleId),
			});
...
export function useToggleGuardRuleActiveMutation() {
	...
	>({
		...
		onSuccess: (_, variables) => {
			...
			toast.success(
				variables.isActive ? "Guard rule activated" : "Guard rule deactivated",
			);
		},
		onError: (error) => {
