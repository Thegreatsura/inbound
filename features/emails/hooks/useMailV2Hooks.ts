...
export const mailV2Keys = {
	...
	outboundEmail: (emailId: string) =>
		[...mailV2Keys.outboundEmails(), emailId] as const,
};
...
export const useUpdateEmailMutation = () => {
	...
	>({
		...
		onMutate: async ({ emailId, updates }) => {
			...
			const previousEmailLists = queryClient.getQueriesData({
				queryKey: mailV2Keys.lists(),
			});
						...
						emails: old.emails.map((email: any) => {
							if (email.id === emailId) {
								...
								if (updates.isRead !== undefined) {
									...
									updatedEmail.readAt = updates.isRead
								...
								if (updates.isArchived !== undefined) {
									...
									updatedEmail.archivedAt = updates.isArchived
										? new Date().toISOString()
										: null;
								}
